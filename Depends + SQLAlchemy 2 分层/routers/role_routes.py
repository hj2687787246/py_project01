...
def create_admin_user_api(
    ...
    """由管理员创建新的管理员账号。"""
    logger.info(
        f"收到创建管理员账号请求: operator_id={current_admin.id}, operator={current_admin.username}, "
        f"username={user.username}, email={user.email}"
    )
    if user_crud.get_user_by_username(db, user.username):
        logger.error(
            f"创建管理员账号失败: operator_id={current_admin.id}, operator={current_admin.username}, "
    ...
    logger.success(
        f"创建管理员账号成功: operator_id={current_admin.id}, operator={current_admin.username}, "
