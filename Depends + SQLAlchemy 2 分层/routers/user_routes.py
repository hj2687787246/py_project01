...
def login_for_access_token(
    ...
    logger.success(
        f"登录成功: user_id={user.id}, username={user.username}, role={user.role}"
    )
...
def create_user_api(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    if crud.get_user_by_email(db, user.email):
        logger.error(
            f"创建用户失败: username={user.username}, email={user.email}, reason=邮箱已存在"
        )
        raise BusinessException(status_code=400, code=4002, message="邮箱已存在")

    # 普通注册入口固定创建 user 角色，避免通过该接口直接注册管理员。
    db_user = crud.create_user(db, user, role_name="user")
    logger.success(
        f"创建用户成功: user_id={db_user.id}, username={db_user.username}, role={db_user.role}"
    )
